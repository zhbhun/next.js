...
export default class Router {
  ...
  async reload (route) {
    ...
    if (route !== this.route) {
      // # 服务端渲染出错时会渲染 pages/_error 组件，这个时候 router 的地址和浏览器实际地址不一致，无法 reload。
  ...
  async change (method, _url, _as, options) {
    ...
    let url = typeof _url === 'object' ? format(_url) : _url
    ...
    // # 增加路由标识
    asQuery._ = Number(parse(window.location.href, true).query._ || '0')
    if (method === 'pushState') {
      asQuery._ += 1
    }
    query._ = asQuery._
    ...
    // # 提前触发 changeState，已解决 push 时浏览器地址没有实时变化的问题
